...
def build_index(selected_types=None):
    ...
    # =========================
    # CREATE INDEX WITH MAPPING (PUT THIS FIRST)
    # =========================
...
def build_index(selected_types=None):
        ...
        body={
            "mappings": {
                "properties": {
                    ...
                    "modified":        {"type": "date"},
                    "content_tokens":  {"type": "keyword"},
                    "content_keyword": {"type": "keyword"}   # ✅ التعديل: أضفنا الحقل ده
                }
            }
        }
    )
    ...
    for file in os.listdir(FOLDER):
        ...
        doc = {
            ...
            "content":         text,
            "content_tokens":  text.lower().split(),
            "content_keyword": text.lower().split()[:50],   # ✅ التعديل: بنحط أول 50 كلمة عشان الـ aggregation
            "filetype":        ext,
            "modified":        datetime.fromtimestamp(os.path.getmtime(path)).strftime("%Y-%m-%d")
        }
